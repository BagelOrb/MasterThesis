...
//		params.netParams.nSamples = params.netParams.batchSize; //new MNIST(true, -1, false).getDataSetSize();
...
//		testSynthNetwork(convAutoEncoderNetwork("-", "5", "6"));
	...
	private static void testSynthNetwork(Network network) {
		...
		evaluateNetworkOnMNIST(network, false, NetworkIO.path+"");
	}
...
//		params1.netParams.staticData = true;
	...
	public static void evaluateNetworkOnMNIST(String... args) {
		...
		evaluateNetworkOnMNIST(network, training, path);
	}
	
	public static void evaluateNetworkOnMNIST(Network network, boolean training, String path) {
		...
		evaluateNetworkOnMNIST(network, training, path, dataset);
	}
	
	static void evaluateNetworkOnMNIST(Network network, boolean training, String path, DataSet dataset) {
