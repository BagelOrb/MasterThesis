...
//		processAllNetworks(base, measurePreSigmoidStats());
		...
		NetworkProcessor q = measurePreSigmoidStats();
		q.process(base);
...
//		NetworkProcessor proc = collectResultsCSV(evaluate);
...
//		appendBatchFiles(args);
		...
		return new NetworkProcessor() {
			...
			public void process(File folder) {
				...
				try {
					...
					resultCodes.add(getResult(folder, null, false)+"");
	...
	public static NetworkProcessor collectResultsCSV(final boolean evaluate) {
		return new NetworkProcessor() {
			
			public ArrayList<Result> results = new ArrayList<Result>(160); 
			
			DatasetInRAM dataset = new DatasetInRAM(new MNIST(false, -1, false));
			...
			public void process(File folder) {
				try {
					...
					Result result = getResult(folder, dataset, evaluate);
					results.add(result);
	...
	private static File getEvalFile(File folder, Network network, DatasetInRAM dataset, boolean evaluate) {
		...
		if (evaluate)
		{
			Main.evaluateNetworkOnMNIST(network, false, folder+"", dataset);
	...
	protected static Result getResult(File folder, DatasetInRAM dataset, boolean evaluate) throws IOException {
		...
		File evalFile = getEvalFile(folder, network, dataset, evaluate);
...
//				}
	...
	public static NetworkProcessor measurePreSigmoidStats() {
		return new NetworkProcessor() {
...
//					Debug.out(((counter+1) *100 / dataset.getLimit())+"%");
				...
				try {
					FileUtils.writeStringToFile(new File(fromFolder+"/stats.txt"), kurtosisString);
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
